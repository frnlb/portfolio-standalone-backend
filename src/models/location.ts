...
import { valuesMapper } from "../helpers/index.ts";
export class LocationModel {
  ...
  static async getLocationByName(name: Location["name"]) {
    ...
    try {
      const query = `SELECT * FROM locations WHERE name="${capName}"`;
      const [results, fields] = await pool.query(query);
      return [results];
    } catch (error) {
  ...
  static async createLocation(location: Location) {
    const [columns, values] = valuesMapper(location);
    let query = `INSERT INTO locations (${columns}) VALUES (${values})`;
    try {
